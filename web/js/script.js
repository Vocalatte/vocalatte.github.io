...
$(function() {
	...
	$(window).resize(function() {
		delay(function() {
				...
				while(1){
					var images_per_row =  gallery_images.length / rows;
					if ( gallery_images.length % rows != 0 ) {
						images_per_row++;
					}
					
					var gallery_width = $('#slide-3 img').width() * images_per_row;
					
					if($('body').width() * 1.5  < gallery_width ){
						rows++;
						continue;
					}
					break;
				...
				var left_pos = $('body').width() - gallery_width;
				left_pos /= 2;
				$('#slide-3 .row').css('left', left_pos + 15);
	...
	arrows.click(function(e) {
		...
		switch( $(this).attr('id') ) {
			...
			case 'arrow-down':
				...
				break;
			case 'arrow-left':
				offset_left = $('#slide-3 .row').offset().left + 200;
				if ( offset_left > 0 ) {
					offset_left = '0px';
				}
				break;
